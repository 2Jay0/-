...
function TodoTemplate() {
  ...
  const onRemoveDoneTodo = (id) => {
    // 7주차 실습 구현
      const RemoveDoneTodo = doneTodos.find((doneTodo) => doneTodo.id === id);

      setDeletedTodos([...deletedTodos, RemoveDoneTodo]);
  ...
  const onToggleDoneToDo = (id) => {
    // 7주차 실습 구현
      const DonetoTodo = doneTodos.find((todo) => todo.id === id);
      DonetoTodo.checked = 0;
      setTodos([...todos, DonetoTodo]);
      setDoneTodos(doneTodos.filter((doneTodo) => doneTodo.id !== id));
